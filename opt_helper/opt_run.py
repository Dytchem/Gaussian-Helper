...
class opt_run:
    def __init__(self, file_name: str, in_file_root: str, out_file_root: str, epoch: int, update_coefficient=1.0, post_process=None):
        ...
        for i in range(1,epoch+1):
            ...
            print(f"Running optimization step {i}: g09 < {in_file_path} > {out_file_path}")
            ...
            if out_file.is_normal():
                ...
                if first_freq > 0:
                    ...
                    if post_process:
                        post_process(in_file)
                    break
                else:
                    print("Updating geometry based on first vibrational mode.")
                    in_file.molecule=in_file.molecule+update_coefficient*Molecule(vib_mol)
                    if post_process:
                        post_process(in_file)
            else:
    ...
    def default_post_process(self, in_file):
        """
        默认后处理函数：重新定向坐标系，使原子 1->2 为 x 轴，原子 3 在 xz 平面。
        """
