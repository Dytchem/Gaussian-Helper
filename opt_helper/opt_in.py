...
class opt_in:
    def __init__(self, file_path: str):
        ...
        atom_end = atom_start
        while atom_end < len(lines) and re.match(r'^[A-Za-z]+\s+[-+]?\d', lines[atom_end].strip()):
            atom_end += 1
    ...
    def get_full_content(self) -> str:
        ...
        """
        lines = self.content.splitlines(keepends=True)
        new_atom_lines = self.molecule.to_string().splitlines(keepends=True)
        lines[self.atom_start:self.atom_end] = new_atom_lines
        return ''.join(lines)
